...
| `locations.json` 스키마 | `{id, seq, name, address, lat, lng}` | `sigungu` 필드 추가 — 시군구 색상 키로 사용. **2026-08-24: 파일 자체를 폐기, SQLite `locations` 테이블로 대체 (아래 "사내 장소 공유 기능" 참조)** |
...
## 사내 장소 공유 기능 (2026-08-24 추가 — 원래 목적 종료 후 피벗)

전파조사 목적이 종료된 뒤, 이 경로 최적화 도구를 사내(회사 전체, 수백~수천 명) 개인/출장 경로 계획 도구로 계속 사용하기 위해 추가. 설계·계획 문서는 `docs/superpowers/specs/2026-08-23-location-sharing-design.md`, `docs/superpowers/plans/2026-08-23-location-sharing.md` 참조.

원안의 "명시적으로 하지 않는 것" 중 4개 항목이 이 피벗으로 뒤집힘 — 원안 문구는 위에 그대로 두고(당시엔 맞는 결정), 여기 실제 현황을 기록:

| 원안의 "하지 않는 것" | 현재 |
|---|---|
| 로그인/회원/팀 공유 | **구현됨** — 사내 이메일 매직링크 로그인(비밀번호 없음) |
...
핵심 설계: 지점은 기본 비공개(작성자만) → 이메일 지정 공유 → 회사 전체공개, 3단계. 조직도/부서 연동 없음(공유는 이메일 직접 지정만). SSO 없음(매직링크만). 기존 KBS 75개 지점(`locations.json`)은 마이그레이션하지 않고 폐기.
