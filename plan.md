...
KBS 전파측정팀이 **75개 지점을 차량으로 나눠 방문**하기 위한 일일 경로 최적화 단일 페이지 웹앱. 운영자 PC에서 Flask를 띄우고, 같은 와이파이의 스마트폰에서 접속해 당일 방문 지점을 지도에서 고른 뒤 OR-Tools로 최적 방문 순서를 계산한다.
...
**구현 완료 (2026-04-17)**. Step 0~4 모두 완료. 실 기기 E2E 테스트(스마트폰 LAN, 인쇄) 일부 미수행.
...
- [x] 검증: `python app.py` → 크롬 `localhost:5000`, 75개 마커 + 시군구별 색상 표시
...
- [x] 검증: 마커 ↔ 사이드바 양방향 sync, 필터, GPS 동작
...
- [ ] 검증: 스마트폰 LAN 접속, 롱프레스, 탭 네비 동작 (**실 기기 미수행**)
- [ ] 검증: 인쇄 미리보기 1페이지 확인 (**브라우저 미수행**)

---

## 최종 E2E 검증

> 코드 구현은 완료. 아래 항목 중 (**실 기기 미수행**) 표시된 것은 스마트폰/앱 환경에서 별도 검증 필요.

- [x] `python tools/xlsx_to_locations.py --xlsx "측정표(배포용)_전북.xlsx" --out locations.json` → 75개 지점
- [x] `pip install -r requirements.txt && python app.py` → 로그에 `prefetch started`, 수초 내 `prefetch done`
- [x] 크롬 `localhost:5000`: 75개 마커 + 시군구별 색상 + 필터 동작
- [x] 마커 클릭 ↔ 사이드바 체크박스 양방향 sync
- [x] 고창군 4개 선택 + 우클릭 출발지 → [최적화] → 번호 마커 + 경로선 + 타임라인, 1초 이내
- [x] Open TSP(도착지 미지정) vs 도착지 지정 — 종단에 맞게 순서 변화 확인
- [x] `OSRM_BASE_URL=http://127.0.0.1:1` → Haversine 폴백 + 근사치 배너 노출
- [ ] 스마트폰 LAN 접속: 지도 롱프레스 메뉴, 탭 네비 동작 (**실 기기 미수행**)
- [ ] 인쇄 미리보기 1페이지 (**브라우저 미수행**)
- [x] `GET /api/health` → OSRM 상태·캐시 크기 반환
- [x] `locations.json` 교체 후 지역 하드코딩 `grep "전북"` 0건
